...
import type { HtmlNodeRenderer, HtmlRegistry } from './types.js';
import {
  renderContainer,
  renderHeading,
  renderText,
...
export function createHtmlRegistry(): Map<IRNodeType, HtmlNodeRenderer> {
  return new Map();
}

const DEFAULT_HTML_RENDERERS = {
...
} satisfies Record<RenderableIRNodeType, HtmlNodeRenderer>;

export function createDefaultHtmlRegistry(): Map<IRNodeType, HtmlNodeRenderer> {
  return new Map(Object.entries(DEFAULT_HTML_RENDERERS) as [IRNodeType, HtmlNodeRenderer][]);
}

export type { HtmlRegistry, HtmlNodeRenderer, RenderChildrenFn, IRNode };
